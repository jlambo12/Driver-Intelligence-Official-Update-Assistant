...
- Hardening pass: operation context now stores stable `OperationId` (no lazy regeneration), logger API uses `LogMessage` envelope, and scan timestamps use `IAppClock`.
- Added boundary error handler (`IUserBoundaryErrorHandler`) so UI/application edges normalize+log errors without coupling UI to infrastructure details.
- Added diagnostics query extension (`ILogDiagnosticsQuery`) and metadata policy object (`MetadataSanitizationPolicy`) for future sink/UI evolution.
