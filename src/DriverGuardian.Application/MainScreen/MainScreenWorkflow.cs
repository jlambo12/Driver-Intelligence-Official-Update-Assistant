    ...
    IShareableReportBuilder reportBuilder) : IMainScreenWorkflow
{
    ...
    private static string BuildVerificationSummary(IReadOnlyCollection<RecommendationDetailResult> recommendationDetails)
    {
        ...
        return waitingForReturnCount > 0
            ? $"Требуется действие пользователя по {waitingForReturnCount} устройств(ам): выполните ручную установку (если ещё не выполнена) и запустите повторный анализ. Повторный анализ покажет, выглядит ли версия драйвера обновлённой или результат пока нельзя подтвердить."
